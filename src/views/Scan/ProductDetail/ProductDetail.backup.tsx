...
// Import real USDA API hooks for consumer transparency
import {
...
function ProductDetail() {
  ...
  // Real USDA data for consumer transparency
  const cropType = historyData?.product?.name?.toLowerCase() || 'corn';
  const farmState = historyData?.parcel?.establishment?.location?.split(',').pop()?.trim() || 'CA';

  // Real USDA API queries for enhanced consumer experience - Only load when user views detailed comparison
  const [showDetailedUSDAData, setShowDetailedUSDAData] = useState(false);
  ...
  const { data: realUSDAData } = useGetUSDAEmissionFactorsQuery(
    ...
    {
      skip: !showDetailedUSDAData || !historyData?.product?.name // Skip unless user requests detailed USDA data
    }
  );
  ...
  const { data: benchmarkData } = useGetUSDABenchmarkComparisonQuery(
    {
      carbon_intensity: (carbonData as any)?.carbon_intensity || 0.001,
      crop_type: cropType,
      state: farmState
    },
    {
      skip: !showDetailedUSDAData || !(carbonData as any)?.carbon_intensity // Skip unless user requests detailed USDA data
    }
  );
  ...
  return (
    <Box minH="100vh" bg={useColorModeValue('gray.50', 'gray.900')}>
      ...
      <ModernProductHeader
        productName={
          (completeData as any)?.farmer?.name || (completeData as any)?.product?.name || 'Product'
        }
        location={
        ...
        confidenceScore={85} // Default confidence score since it's not in the interface
        isUSDAVerified={carbonData?.isUsdaVerified || false}
        eventsCount={(completeData as any)?.events?.length || 0}
        productionId={productionId!}
        onEducationOpen={handleEducationOpen}
        onReviewOpen={() => {}} // Add your review handler
        onAuthPromptOpen={() => {}} // Add your auth prompt handler
        onShare={handleShare}
        isAuthenticated={false} // Update this based on your auth state
      />
      ...
      <Container maxW="container.xl" py={8}>
        <SimpleGrid columns={{ base: 1, lg: 3 }} spacing={8}>
          ...
          <Box gridColumn={{ base: "1", lg: "1 / 3" }}>
            ...
            {/* Production Journey */}
            <Card bg={useColorModeValue('white', 'gray.800')} shadow="lg" borderRadius="xl" mb={8}>
              <CardHeader>
                <HStack spacing={3}>
              ...
              <CardBody pt={0}>
                {(() => {
                  ...
                  const historyEvents =
                    (completeData as any)?.production_events ||
                    (completeData as any)?.history_events ||
                    (completeData as any)?.events ||
                    [];
                    ...
                    const mapEventsToTimeline = (apiEvents: any[]) => {
                      return apiEvents.map((event, index) => {
                        return {
                          ...
                          date: event.date || event.created_at || new Date().toISOString(),
              ...
              <>
                <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={8} mb={10}>
                  <Box>
                    ...
                    {/* Government Verification Section - NEW ENHANCEMENT */}
                        ...
                        <VStack spacing={4} align="stretch">
                          <HStack spacing={3} justify="center">
                            ...
                            <Badge
                              ...
                              py={1}
                              borderRadius="full">
                              {realUSDAData.api_status.data_source}
                            </Badge>
                          </HStack>
                          ...
                          <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                            ...
                            <Box bg="white" p={4} borderRadius="md" boxShadow="sm">
                              <VStack spacing={2} align="center">
                                <Icon as={FaAward} color="purple.500" boxSize={6} />
                                <Text fontSize="sm" fontWeight="bold" color="gray.700">
                                  Carbon Credibility Score
                                </Text>
                                <Text fontSize="lg" fontWeight="bold" color="green.600">
                                  {realUSDAData.usda_credibility.score}/100
                                </Text>
                                <Badge
                                  colorScheme={
                                    realUSDAData.usda_credibility.score >= 90 ? 'green' : 'yellow'
                                  }>
                                  {realUSDAData.usda_credibility.rating}
                                </Badge>
                              </VStack>
                            </Box>
                          </SimpleGrid>
                            ...
                            <Box bg="white" p={4} borderRadius="md" boxShadow="sm">
                              <VStack spacing={3}>
                                <HStack justify="space-between" w="full">
                                  <Text fontSize="sm" fontWeight="bold" color="gray.700">
                                    Farm Performance vs Regional Average
                                  </Text>
                                  <Badge
                                    colorScheme={
                                ...
                                <Text fontSize="xs" color="gray.600" textAlign="center">
                                  ...
                                  % below regional average - Excellent performance!
                                </Text>
                              </VStack>
                            </Box>
                          )}

                          {nutritionalData?.success && (
                            <Box bg="white" p={4} borderRadius="md" boxShadow="sm">
                              <VStack spacing={3}>
                                <HStack spacing={2} justify="center">
                                  <Icon as={FaAppleAlt} color="orange.500" boxSize={5} />
                                  <Text fontSize="sm" fontWeight="bold" color="gray.700">
                                    Nutritional Carbon Efficiency (USDA FoodData Central)
                                  </Text>
                                </HStack>
                                <SimpleGrid columns={3} spacing={4} w="full">
                                  {Object.entries(
                                    nutritionalData.nutritional_analysis.carbon_efficiency
                                  )
                                    .slice(0, 3)
                                    .map(([key, value]) => (
